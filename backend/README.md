...
1. **Private Key Security**: Use secrets manager (AWS Secrets Manager, etc.)
2. **Rate Limiting**: Add rate limiting to prevent abuse
3. **Authentication**: Implement API key or JWT auth
4. **CORS**: Configure CORS for production domains only
5. **HTTPS**: Use HTTPS in production (reverse proxy with nginx/caddy)
6. **Database Backups**: Regular SQLite database backups

### Monitoring

Monitor these metrics:
- Backend wallet USDFC balance
- Synapse allowance remaining
- Upload success/failure rates
- User balance credits
- API response times

Set up alerts for:
- Low backend wallet balance
- Failed uploads
- API errors
...
- **Render**: Auto-deploy from git, free tier
...
Common issues:
- **"Synapse SDK not initialized"**: Check private key and RPC connection
- **"Insufficient balance"**: User needs to bridge more tokens
- **"Upload failed"**: Check backend wallet has USDFC and FIL for gas
